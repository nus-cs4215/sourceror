...
function read_js_result(linear_memory: WebAssembly.Memory): any {
	...
	switch (tag) {
		...
		case 4: {
			...
			return extractStringFromMemory(ptr, linear_memory, mem);
		}
		case 5:
		...
		case 6: // array
			...
			const raw = new Uint8Array(linear_memory.buffer, ptr + 4, len * 12);

			const chunkedArray = _.chunk(raw, 12);
			...
			const result = chunkedArray.map((element, i) => {
				...
				const data = _.slice(element, 4);
				return parseDataByTag(data, tag, linear_memory);
			});
...
function parseDataByTag(
	...
	switch (tag) {
		...
		case 4: // string
			_.reverse(data);
			const ptr = parseInt(int64ToString(data, true));
			return extractStringFromMemory(ptr, linear_memory, mem);
...
function extractStringFromMemory(
	ptr: number,
	linear_memory: WebAssembly.Memory,
	mem: DataView
) {
	...
	return decoder.decode(
		new Uint8Array(linear_memory.buffer, ptr + 4, len) // First 4 bytes store the length of the string
	);
}

function int64ToString(bytes: number[], isSigned: boolean) {
	const isNegative = isSigned && bytes.length > 0 && bytes[0] >= 0x80;
	const digits: number[] = [];
	bytes.forEach((byte, j) => {
		if (isNegative) byte = 0x100 - (j == bytes.length - 1 ? 0 : 1) - byte;
